...
## 추가 노트 (2026-06-15) — 실제 구현 결과

MVP 구현 단계에서 화면이 3개(홈/약관/피싱)로 확정되어 화면 간 상태 공유 요구가
거의 없었고, 위 "후속 작업"(Provider 도입)을 진행하지 않고 **`setState`만으로
충분**했다. 위 "대안 A: setState"에서 우려했던 화면 간 공유 문제가 실제로는
발생하지 않은 것.

Provider 마이그레이션 경로는 본 ADR의 분석(대안 B)을 그대로 따르며, 앱 규모가
커져 화면 간 상태 공유가 필요해지는 시점에 재검토한다. 현재 구조 설명은
`docs/architecture.md`를 참고.
